...
- [breedsim-mcp](https://github.com/musharna/breedsim-mcp) - Breeding-scheme simulation over MCP, driving AlphaSimR. Enforces a replicate floor and returns per-cycle mean, sd and confidence interval rather than a single stochastic run, so an agent cannot quote simulation noise as a measured gain.
- [phylokit-mcp](https://github.com/musharna/phylokit-mcp) - Phylogenetic inference over MCP, driving IQ-TREE 2 through piqtree. Always runs a bootstrap and returns per-clade support alongside the topology, since a maximum-likelihood tree looks identical whether or not the data support it.
- [plantcv-mcp](https://github.com/musharna/plantcv-mcp) - PlantCV as an MCP measurement instrument: returns plant trait values together with the segmentation overlay they were computed from, and refuses to return numbers when the segmentation is degenerate.
- [data-aggregator-mcp](https://github.com/musharna/data-aggregator-mcp) - One search across 17 data sources (Zenodo, DataCite, NCBI GEO/SRA, BioStudies, PubMed, CZ CELLxGENE, RCSB PDB, GWAS Catalog, GBIF and more), deduplicated and normalized behind a single resource model; resolve any hit to its file manifest and citation, then fetch it to disk with checksum verification.
